function [Gc, T_i, T_d, T_f, Kc] = pidfPoleCancellation(Gp, push_gain)
    ...
    p = pole(Gp);
    if length(p) ~= 2
        error('Le procédé doit être du 2e ordre.');
    end
    
    % On s'assure que les pôles sont réels et négatifs
    if any(imag(p) ~= 0) || any(real(p) >= 0)
        error('Les pôles du procédé doivent être réels et négatifs');
    end
    ...
    % Calcul du gain du régulateur Kc 
    Kc = b / (4 * zeta^2 * k_p * T_f)*push_gain;

    % Construction du PIDF 
    ...
    % Affichage
    fprintf('Paramètres calculés :\n');
    fprintf('  T_i = %.4g,  T_d = %.4g,  T_f = %.4g, Kc = %.4g\n', T_i, T_d, T_f, Kc);
end