...
fin = 203; % ajustter pour la fin de l'échelon
couplage_tec = 3.15; % le couplage et le gain du tec par rapport au voltage
T_air = 298.15; % ajuster la température de la pièce
%%%%%%%%%%%%%%%%

%res_exp = read_csv("data_2025-02-16.csv");
res_data = read_csv("Test(-0.45V).csv");
%t3_exp = res_exp.Temp_0__C_(cut:fin, 1);
t3_exp = res_data.T3(cut:fin, 1);

%t2_exp = res_exp.Temp_1__C_(cut:fin, 1);
t2_exp = res_data.T2(cut:fin, 1);

%t1_exp = res_exp.Temp_2__C_(cut:fin, 1);
t1_exp = res_data.T1(cut:fin, 1);
...
% tesmps et tension
%temps_exp =  res_exp.Temps_s_(cut:fin, 1) - res_exp.Temps_s_(cut, 1);
temps_exp =  res_data.Temps(cut:fin, 1) - res_data.Temps(cut, 1);
%tension = res_exp.DeltaV_V_(cut:fin, 1); 
%tension = res_data.Tension(cut:fin, 1);
tension =  zeros(length(t3_exp),1);
tension(:)  = -0.45;
k_amp = 1.01;
u_exp = tension*k_amp;
...
%%%%%%%%%%%%%%%%5
% Peut-être à ajuster si essaie < 1000 points
Pin = mean(u_exp(1:169))*couplage_tec;
%%%%%%%%%%%%%
h_conv = 12.4;
k = 215;
cp = 897;
