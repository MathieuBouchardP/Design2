...
%% Lire le contenu du fichier JSON
   ...
   % subplot(131)
    f_1 = surf(1000*X, 1000*Y, T-273.15);
    ...
    view(3); 
    axis vis3d; 
    timeText = text(0.05, 0.95, 1.05, '', 'Units', 'normalized', 'FontSize', 12, 'Color', 'k', 'BackgroundColor', 'w');
  %  hold on

 %   f1_therm1 = plot(Temps(1), thermistance1(1) - 273.15, 'g', 'DisplayName', 'Thermistance 1');
 %   f1_therm2 = plot(Temps(1), thermistance2(1) - 273.15, 'r', 'DisplayName', 'Thermistance 2');
%    f1_therm3 = plot(Temps(1), thermistance3(1) - 273.15, 'b', 'DisplayName', 'Thermistance 3');
%    xlabel('Temps [s]');
...
%    grid on;
 ...
 %   subplot(133);
 %   f1_energy_added = plot(Temps(1), energy_added(1), 'g', 'DisplayName', 'Energie déposée');
 %   f1_energy_loss = plot(Temps(1), energy_loss(1), 'r', 'DisplayName', 'Energie dissipée');
 %   xlabel('Temps [s]');
 %   ylabel('Énergie dans l''itération');
 ...
 %   grid on;
    ...
    function updateDisplay(T, dt, timeText, f_1)
        % Met à jour l'affichage dans un thread parallèle
        set(f_1, 'ZData', T - 273.15); % Met à jour la surface
        set(timeText, 'String', ['Temps : ' num2str(dt, '%.2f') ' s']);
        drawnow limitrate;
    end
% Boucle principale
for t = 1:Nt
    ...
    f = parfeval(pool, @updateDisplay, 0, T, t*dt, timeText, f_1);
    
    % Pour vérifier si la tâche est terminée avant de continuer
    % Vous pouvez aussi utiliser 'fetchOutputs(f)' pour récupérer les résultats si besoin.
   ...
   % if mod(t, round(Nt/1000)) == 0 || t==1
   %     figure(f1);
   %     set(f_1, 'ZData', T-273.15);
    %    set(timeText, 'String', ['Temps : ' num2str(t*dt, '%.2f') ' s']);
%        set(f1_therm1, 'YData', thermistance1(1:t) - 273.15);
 %       set(f1_therm2, 'YData', thermistance2(1:t) - 273.15);
  %      set(f1_therm3, 'YData', thermistance3(1:t) - 273.15);
  %      set(f1_therm1, 'XData', 1:t);
  %      set(f1_therm2, 'XData', 1:t);
  %      set(f1_therm3, 'XData', 1:t);
  %%      set(f1_energy_added, 'YData', energy_added(1:t));
   %     set(f1_energy_loss, 'YData', energy_loss(1:t));
   %     set(f1_energy_added, 'XData', 1:t);
   %     set(f1_energy_loss, 'XData', 1:t);
